...
public: 
	...
	double prepareMazeForRunner(MazeGenerator Level[][MAZESIZE]); // Changed to double to get time into mazeToFile function
...
void MazeGenerator::runMazeGen(MazeGenerator Level[][MAZESIZE]) {
	...
	do {
		...
		switch (choice) {
		case 1:
			//prepareMazeForRunner(Level);//Called in mazeToFile
			mazeToFile(Level);
			break;
		case 2:
...
double MazeGenerator::prepareMazeForRunner(MazeGenerator Level[][MAZESIZE]) {//Timer here
	int posX = 0;
	int posY = 0;
	int goalX = 0;
	int goalY = 0;
...
void MazeGenerator::mazeToFile(MazeGenerator Level[][MAZESIZE]) {//Adds timer result to file
	int response;
	ofstream out;
	char file[100];
	...
	if (response == 1) {
		...
		out << "\n" << d << " seconds" << endl;//Result added here
